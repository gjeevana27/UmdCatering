...
# ---------------------------------------------------------------------------
def _render_title_with_egg_drop():
    """The page title, plus a purely decorative egg that repeatedly
    falls from above the page onto the plate emoji and becomes a fried
    egg on impact -- a little squish/rebound sells the landing, then
    it holds for a while before fading out and resetting to fall
    again. No functional change to the title itself.

    Wrapped in a REAL st.container(key=...) (not raw injected HTML,
    which can't be nested around other Streamlit widgets the way a
    real container can -- same reasoning as the Ask FAB's own
    container) so the egg can be positioned absolutely relative to the
    title's own box via plain CSS, landing in roughly the same spot
    regardless of viewport width, and scrolling naturally WITH the
    page content since it's a normal absolutely-positioned element,
    not position:fixed to the viewport."""
    st.markdown("""
<style>
div.st-key-title_wrap{position:relative;}
/* Streamlit wraps the st.markdown() call below in its OWN element
   container div, which Streamlit's base styles give position:relative
   -- that becomes .egg-drop's nearest positioned ancestor instead of
   title_wrap, since CSS position:absolute anchors to the CLOSEST
   positioned ancestor, not any arbitrary one further up the tree.
   Confirmed via a live DOM inspection (offsetParent was that inner
   wrapper, not title_wrap) after the egg first rendered well below the
   title, overlapping the caption underneath it. Forcing this one
   specific wrapper back to static undoes that, without touching how
   any other stElementContainer on the page behaves. */
div.st-key-title_wrap .stElementContainer:has(.egg-drop){position:static !important;}
.egg-drop{position:absolute; top:30px; left:6px; z-index:5; font-size:2.1rem;
line-height:1; pointer-events:none;
animation:egg-fall 7s linear infinite;}
/* linear overall timing, deliberately -- a single easing curve applied
   to the WHOLE animation (fall + squish + rebound + hold) distorts
   every segment by the same curve, not just the fall, which made the
   brief squish/rebound keyframes warp unpredictably. Gravity-like
   acceleration is faked instead by spacing the FALL keyframes
   themselves unevenly -- barely moving over the first 20% of the
   cycle, covering most of the distance in the last 15% -- while every
   other segment (squish/rebound/hold) stays evenly spaced and
   reads correctly. */
@keyframes egg-fall{
  0%   {transform:translateY(-130px) scale(1); opacity:0;}
  4%   {opacity:1;}
  20%  {transform:translateY(-118px) scale(1);}
  30%  {transform:translateY(-96px) scale(1);}
  36%  {transform:translateY(-58px) scale(1);}
  39%  {transform:translateY(-18px) scale(1);}
  41%  {transform:translateY(0px) scale(1);}
  43%  {transform:translateY(2px) scale(1.25,0.7);}
  47%  {transform:translateY(-3px) scale(0.9,1.1);}
  51%  {transform:translateY(0px) scale(1,1);}
  90%  {transform:translateY(0px) scale(1,1); opacity:1;}
  100% {transform:translateY(-130px) scale(1); opacity:0;}
}
/* Both glyphs sit stacked in the exact same spot (the second
   absolutely positioned over the first); only their own opacity
   crossfades right at impact (~41%), swapping which one is visible --
   the shared parent's fall/squish/opacity above still drives where
   and how big BOTH appear, this only picks which glyph is shown. */
.egg-drop .egg-raw, .egg-drop .egg-cooked{display:block;}
.egg-drop .egg-cooked{position:absolute; top:0; left:0; opacity:0;
animation:egg-cook 7s linear infinite;}
.egg-drop .egg-raw{animation:egg-raw-fade 7s linear infinite;}
@keyframes egg-cook{0%,39%{opacity:0;} 41%,100%{opacity:1;}}
@keyframes egg-raw-fade{0%,39%{opacity:1;} 41%,100%{opacity:0;}}
</style>
""", unsafe_allow_html=True)
    with st.container(key="title_wrap"):
        st.title("🍽️ Good-to-Go")
        st.markdown(
            '<div class="egg-drop"><span class="egg-raw">🥚</span>'
            '<span class="egg-cooked">🍳</span></div>',
            unsafe_allow_html=True,
        )


_render_title_with_egg_drop()
